...
We will see a quick setup to configure the ``netbox`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``netbox`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``netbox`` formulas under ``/srv/salt/netbox/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/netbox/parameters/dns:domain/
    ln -s dns:domain /srv/salt/netbox/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/netbox/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/netbox/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``netbox`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``netbox`` formulas under ``/srv/salt/netbox/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/netbox/parameters/roles
...
We create a configuration for the role ``netbox/server`` in ``/srv/salt/netbox/parameters/roles/netbox/server.yaml``:
...
We create another configuration for the role ``netbox/client`` in ``/srv/salt/netbox/parameters/roles/netbox/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles netbox/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles netbox/client
...
.. code-block:: console
    ...
    mv /srv/salt/netbox/parameters/dns:domain/  '/srv/salt/netbox/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    {%- from tplroot ~ "/map.jinja" import mapdata as netbox with context %}

    test-does-nothing-but-display-netbox-as-json:
      test.nop:
        - name: {{ netbox | json }}
...
Here is an example based on `template-formula/netbox/config/file.sls`_:
...
.. code-block:: sls
    ...
    netbox-config-file-file-managed:
      file.managed:
        - name: {{ netbox.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='netbox-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ netbox.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``netbox`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ netbox['winner'] }}
...
.. _template-formula/netbox/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/netbox/config/file.sls
